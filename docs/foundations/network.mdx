...
There's a proposed replacement for the 2.4 and 5.8 GHz rockets, a single MikroTik NetMetal ax per side that runs both bands at once, along with the antennas it forces you to buy and why. That's all on [Signals](./signals).
...
The current EIGRP setup is solid wired technology, but it struggles on a mobile, lossy wireless link. The proposed fix is [Babel](./babel), running either on a pair of Raspberry Pi edge routers hung off these switches, or on a pair of MikroTik RB5009s that take over their routing role entirely.
