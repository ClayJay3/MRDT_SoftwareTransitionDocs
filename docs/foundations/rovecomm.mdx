...
packet-beta
0-7: "Version (=3)"
8-23: "Data ID (16-bit, big-endian)"
24-39: "Data Count (16-bit, big-endian)"
40-47: "Data Type (enum)"
48-95: "Payload, Count × sizeof(type), network byte order"
```

Every packet starts with the same 6 bytes. The version is currently `3`, and if it doesn't match you get an `INVALID_VERSION` reply back. The data ID is what the packet actually is, like `11000` for StartAutonomy, and the IDs are namespaced per board by range. The data count is the number of elements, not the number of bytes, and the data type is one of the values in the table below. Every element gets converted to network byte order with `htons`/`htonl`/`htonll` before it goes out.
