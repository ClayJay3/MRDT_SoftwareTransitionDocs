...
import FrameVisualizer from '@site/src/components/visuals/FrameVisualizer';
...
Drag the heading and the detection position below to see that transform happen. This is the single most common source of navigation bugs, so it's worth getting a feel for which way the world moves when the rover turns.
...
| Pure Pursuit | search pattern | a fast, lightweight path-follower for the spiral search |
